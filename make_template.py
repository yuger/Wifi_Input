...
def parse(lang, values):
  ...
  if lang == 'en':
    f=open('WifiInput/src/main/res/values/strings.xml');
  else:
    f=open('WifiInput/src/main/res/values-%s/strings.xml' % lang);
  p.ParseFile(f);
...
parse('en', values_en);
parse_R('WifiInput/build/generated/source/r/debug/com/wxy/wifiinput/R.java', values_lang);
...
old = None
try:
  old = open("WifiInput/src/main/res/raw/key.html").read();
except:
  pass
if (old != page):
  open("WifiInput/src/main/res/raw/key.html", "w").write(page.encode('UTF-8'));
