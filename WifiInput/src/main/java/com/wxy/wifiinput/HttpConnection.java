...
abstract class HttpConnection {
    ...
    class ConnectionFailureException extends RuntimeException {
        ...
        ConnectionFailureException(String msg) {
            super(msg);
        }
    }

    HttpConnection(SocketChannel ch) {
        this.ch = ch;
        Log.d("WiFiInput", "new connection");
    }
     ...
     */
    final ConnectionState newInput() throws IOException {
        if (in.hasRemaining()) {
    ...
    final ConnectionState newOutputBuffer() throws IOException {
        ...
        while (true) {
            switch (httpConnectionState) {
                ...
                case LOOK_REQUEST_HEADERS:
                    ...
                    if (headerMatcher == null) httpConnectionState = HttpConnectionState.SKIP_HEADERS;
                    else {
    ...
    private byte[][] header = new byte[2][256];
    private int[] headerLen = new int[2];
    ...
    private HeaderState matchHeaderName() {
        ...
        for (i = offset; i < limit; ) {
            ...
            while (true) {
                ...
                while (true) {
                    matchedMatcher++;
                    if (matchedMatcher >= npatterns) return HeaderState.SKIP_HEADER_NAME;
    ...
    private HeaderState storeHeaderData() {
        ...
        for (int i = offset; i < limit; i++) {
            byte b = data[i];
            if (b == LETTER_CR) {
//        Log.d("WiFiInput", "header value = " + new String(currentHeader, 0, currentHeaderLen));
    ...
    private HttpConnectionState readFormData() {
        ...
        for (int i = offset; i < limit; i++) {
            ...
            if (formDataLength == formDataExpectedLength) {
                ...
                this.offset = i + 1;
//        Log.d("WiFiInput", "Form data: " + new String(formData, 0, formDataLength));
    ...
    void setKey(SelectionKey clientkey) {
        this.key = clientkey;
    }
}
