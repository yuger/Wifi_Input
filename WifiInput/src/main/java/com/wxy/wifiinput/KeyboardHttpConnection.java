...
final class KeyboardHttpConnection extends HttpConnection {
    ...
    private static final byte[] Q_ICON_PNG = "icon.png".getBytes();// 浏览器 favico图标
    ...
    private HeaderMatcher formHeaders = new HeaderMatcher("Content-Type", "Content-Length");

    KeyboardHttpConnection(final KeyboardHttpServer server, SocketChannel ch) {
        super(ch);
        this.server = server;
    }
...
//    Log.d("WiFiInput", "req: " + new String(request, 0, requestLength));
        queryEnd = 0;
    ...
    ByteBuffer sendData(String content_type, byte[] content, int content_length) {
    ...
    private ByteBuffer sendImage(int resid) {
    ...
    private ByteBuffer onIconPngRequest() {
        return sendImage(R.drawable.icon);
    }
    ...
    private ByteBuffer onKeyRequest() {
        String response = server.processKeyRequest(new String(request, cmdEnd + 1, queryEnd));
